...
export default function LoverProfilePage() {
  ...
  const [loverProfile, setLoverProfile] = useState<LoverProfile | null>(null)
  ...
  const fetchProfile = async () => {
    try {
      const [userRes, loverRes] = await Promise.all([
        fetch('/api/users/me'),
        fetch('/api/profiles/lover'),
      ])
      ...
      if (loverRes.ok) {
        const data = await loverRes.json()
        setLoverProfile({
    ...
    try {
      const [userRes, loverRes] = await Promise.all([
        ...
        fetch('/api/profiles/lover', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(loverProfile),
        }),
      ])

      if (userRes.ok && loverRes.ok) {
    ...
    <div>
      ...
      <Card>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Pet Lover Specific Fields */}
          <div className="border-t pt-6 mt-6">
            <h3 className="text-lg font-semibold mb-4">Pet Lover Profile</h3>

            <div className="space-y-4">
              <Textarea
                label="Experience"
                value={loverProfile?.experience || ''}
                onChange={(e) => setLoverProfile(prev => prev ? { ...prev, experience: e.target.value } : null)}
                placeholder="Tell pet owners about your experience with pets..."
                rows={3}
              />

              <Textarea
                label="Certifications"
                value={loverProfile?.certifications || ''}
                onChange={(e) => setLoverProfile(prev => prev ? { ...prev, certifications: e.target.value } : null)}
                placeholder="List any pet care certifications you have..."
                rows={2}
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input
                  ...
                  type="number"
                  value={loverProfile?.hourlyRate || ''}
                  onChange={(e) => setLoverProfile(prev => prev ? { ...prev, hourlyRate: e.target.value ? Number(e.target.value) : null } : null)}
                ...
                <Input
                  ...
                  type="number"
                  value={loverProfile?.serviceRadius || ''}
                  onChange={(e) => setLoverProfile(prev => prev ? { ...prev, serviceRadius: e.target.value ? Number(e.target.value) : null } : null)}
              ...
              <Textarea
                label="Availability"
                value={loverProfile?.availability || ''}
                onChange={(e) => setLoverProfile(prev => prev ? { ...prev, availability: e.target.value } : null)}
