...
export default function GroomerProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      const [userRes, groomerRes] = await Promise.all([
        fetch('/api/users/me'),
        fetch('/api/profiles/groomer'),
      ])
      ...
      if (groomerRes.ok) {
        const data = await groomerRes.json()
        setGroomerProfile({
    ...
    try {
      const [userRes, groomerRes] = await Promise.all([
        ...
        fetch('/api/profiles/groomer', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(groomerProfile),
        }),
      ])

      if (userRes.ok && groomerRes.ok) {
      ...
      <Card>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Input
            ...
            placeholder="your_salon_instagram"
          />
          ...
          <div className="border-t pt-6 mt-6">
            ...
            <div className="space-y-4">
              <Input
                label="Salon Name"
                value={groomerProfile?.salonName || ''}
                onChange={(e) => setGroomerProfile(prev => prev ? { ...prev, salonName: e.target.value } : null)}
                placeholder="Enter your salon name"
              />

              <Textarea
                label="Salon Address"
                value={groomerProfile?.salonAddress || ''}
                onChange={(e) => setGroomerProfile(prev => prev ? { ...prev, salonAddress: e.target.value } : null)}
              ...
              <Input
                label="Salon Phone"
                value={groomerProfile?.salonPhone || ''}
                onChange={(e) => setGroomerProfile(prev => prev ? { ...prev, salonPhone: e.target.value } : null)}
                placeholder="Salon contact number"
              />

              <Textarea
                label="Business Hours"
                value={groomerProfile?.hours || ''}
                onChange={(e) => setGroomerProfile(prev => prev ? { ...prev, hours: e.target.value } : null)}
                placeholder="e.g., Mon-Sat: 10am-7pm, Sun: Closed"
                rows={2}
              />

              <Textarea
                label="Services & Pricing"
                value={groomerProfile?.services || ''}
                onChange={(e) => setGroomerProfile(prev => prev ? { ...prev, services: e.target.value } : null)}
                placeholder="e.g., Basic Bath - Rs 500, Full Grooming - Rs 1000, Nail Trim - Rs 200"
                rows={3}
              />
            </div>
          </div>
          ...
          <Textarea
            ...
            placeholder="Tell pet owners about yourself and your grooming experience..."
            rows={3}
          />
