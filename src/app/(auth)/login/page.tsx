...
import { isNativePlatform, nativeGoogleSignIn, nativeAppleSignIn } from '@/lib/capacitor'
...
function LoginForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (isNativePlatform()) {
        // Use native Google Sign-In SDK on mobile
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <button
        ...
        className="w-full bg-black text-white py-4 px-6 rounded-2xl font-semibold flex items-center justify-center gap-3 active:scale-[0.98] transition-transform disabled:opacity-50"
      >
        {isAppleLoading ? (
          <LoadingSpinner size="sm" />
        ) : (
